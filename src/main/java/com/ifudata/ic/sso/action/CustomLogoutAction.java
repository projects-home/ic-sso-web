package com.ifudata.ic.sso.action;
...
/**
 * 自定义登出action，处理登出service异常情况.
 */
@Component("customLogoutAction")
public class CustomLogoutAction extends AbstractLogoutAction{
    ...
    protected Event doInternalExecute(final HttpServletRequest request, final HttpServletResponse response,
        ...
        if (this.followServiceRedirects && service != null) {
            final RegisteredService rService = this.servicesManager.findServiceBy(new WebApplicationServiceFactory().createService(service));

            if (rService != null && rService.getAccessStrategy().isServiceAccessAllowed()) {
                context.getFlowScope().put("logoutRedirectUrl", service);
            }else{
                final String msg = String.format("ServiceManagement: Unauthorized Service Access. "
                        + "Service [%s] is not found in service registry.", service);
                logger.warn(msg);
